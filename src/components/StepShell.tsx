...
export function StepShell({
  ...
  const phaseIdx = phases.findIndex(
    (p, i) => p === currentPhase && (i === phases.length - 1 || p !== phases[i + 1] || true),
  );
      ...
      <header className="sticky top-0 z-30 border-b bg-card/95 backdrop-blur">
        <div className="mx-auto grid max-w-6xl grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link
              to="/"
              className="hidden rounded-xl px-3 py-1.5 text-sm font-bold text-muted-foreground hover:bg-muted sm:inline"
            >
              Devices
            </Link>
          </div>
      ...
      <main className="mx-auto grid max-w-6xl gap-6 px-4 pt-5 lg:grid-cols-[minmax(0,440px)_1fr] lg:gap-10">
        <section aria-label="Practice demo" className="lg:sticky lg:top-28 lg:self-start">
          ...
          >
            ...
            <div className="border-t p-4 group-open:block lg:block lg:border-t-0 lg:p-5">
              {left}
            </div>
          </details>
        </section>
