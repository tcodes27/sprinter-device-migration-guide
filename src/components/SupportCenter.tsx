          ...
          <div className={cn("border-b px-5 pb-4 pt-5 sm:px-6 sm:pt-6", view === "stop" && "rounded-t-3xl bg-danger-soft")}>
            <DialogHeader className="text-left">
              ...
              <DialogDescription className="text-base">
                ...
                {view === "request" && "We've filled in where you are so you don't have to explain."}
                {view === "stop" && "You don't need to guess or keep trying. Field Support can take it from here."}
                {view === "sent" && "Field Support will contact you shortly. You can keep your device where it is."}
              </DialogDescription>
            </DialogHeader>
          </div>
          ...
          <div className="min-h-0 overflow-y-auto px-5 pb-6 pt-4 sm:px-6">
            <AnimatePresence mode="wait" initial={false}>
              <motion.div key={view} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.18 }}>
                ...
                {view === "request" && <RequestView location={location} issue={issue} onIssue={setIssue} onSend={() => setView("sent")} onBack={close} />}
                {view === "stop" && (
                  <StopView
                    location={location}
                    onContact={() => {
...
function DemoBadge() {
  return <span className="rounded-full bg-warning-soft px-2.5 py-0.5 text-[11px] font-extrabold uppercase tracking-wide text-warning-foreground">{supportCopy.demoBadge}</span>;
}

function LocationCard({ location, compact = false }: { location: SupportLocation | null; compact?: boolean }) {
  if (!location) {
    return (
...
function MenuView({ location, onPick, onClose }: { location: SupportLocation | null; onPick: (v: SupportView) => void; onClose: () => void }) {
  return (
    <div className="space-y-4">
      <LocationCard location={location} />
      <div className="grid gap-2">
        <Button size="xl" variant="outline" className="justify-start" onClick={() => onPick("chat")}>
          <MessageCircle aria-hidden /> Chat with Field Support
        </Button>
        <Button size="xl" variant="outline" className="justify-start" onClick={() => onPick("request")}>
          <MessageSquareText aria-hidden /> Text Field Support
        </Button>
        <Button size="xl" variant="destructive" className="justify-start" onClick={() => onPick("stop")}>
      ...
      <p className="text-center text-xs font-semibold text-muted-foreground">Your progress is saved. Nothing resets when you ask for help.</p>
    </div>
  );
}

const afterOptions = ["My device is not working correctly", "I'm not sure if I did everything right", "I'm having connectivity problems", "Something else"];
...
function AfterView({ onPick, onClose }: { onPick: (issue: string) => void; onClose: () => void }) {
  return (
    <div className="space-y-4">
      <div className="grid gap-2">
        {afterOptions.map((o) => (
          <Button key={o} size="xl" variant="outline" className="justify-start whitespace-normal text-left" onClick={() => onPick(o)}>
            {o}
          </Button>
        ))}
      </div>
...
function ChatView({ location, onRequest, onClose }: { location: SupportLocation | null; onRequest: (issue: string) => void; onClose: () => void }) {
  ...
  return (
    <div className="space-y-3">
      <div className="max-h-72 space-y-2 overflow-y-auto rounded-2xl bg-muted/60 p-3" aria-live="polite">
        {msgs.map((m, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className={cn("flex", m.from === "me" ? "justify-end" : "justify-start")}>
            <p className={cn("max-w-[85%] rounded-2xl px-4 py-2.5 text-base font-semibold", m.from === "me" ? "rounded-br-md bg-primary text-primary-foreground" : "rounded-bl-md bg-card shadow-soft")}>{m.text}</p>
          </motion.div>
        ))}
        {typing && (
          <div className="flex justify-start">
            <span className="flex gap-1 rounded-2xl rounded-bl-md bg-card px-4 py-3 shadow-soft" aria-label="Field Support is typing">
              {[0, 1, 2].map((i) => (
                <motion.span key={i} className="h-2 w-2 rounded-full bg-muted-foreground" animate={{ opacity: [0.3, 1, 0.3] }} transition={{ repeat: Infinity, duration: 1, delay: i * 0.15 }} />
              ))}
            </span>
          </div>
        )}
      ...
      <div className="flex flex-wrap gap-2">
        {quick.map((q) => (
          <button key={q.label} type="button" onClick={() => send(q.label, q.reply)} className="rounded-full border bg-card px-3 py-1.5 text-sm font-extrabold text-primary hover:bg-primary-soft">
            {q.label}
          </button>
        ))}
      </div>
      ...
      >
        <input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="Type a message…" aria-label="Message Field Support" className="min-h-12 flex-1 rounded-2xl border bg-card px-4 text-base font-semibold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/40" />
      ...
      <p className="text-center text-xs font-semibold text-muted-foreground">{supportCopy.demoNote}</p>
    </div>
  );
}

function RequestView({ location, issue, onIssue, onSend, onBack }: { location: SupportLocation | null; issue: string; onIssue: (s: string) => void; onSend: () => void; onBack: () => void }) {
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-2">
        ...
        <Field label="Current step" value={location?.currentAction ? `${stepText} · ${location.currentAction}` : stepText} />
        <div className="rounded-2xl bg-muted/70 p-3">
          <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-muted-foreground">Issue</p>
          <input value={issue} onChange={(e) => onIssue(e.target.value)} placeholder="What's happening?" aria-label="Issue" className="mt-1 w-full bg-transparent text-base font-extrabold focus-visible:outline-none" />
        </div>
      </div>

      {showMsg ? (
        <Textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Anything else Field Support should know?" className="min-h-24 rounded-2xl text-base font-semibold" />
      ) : (
        <Button variant="outline" size="lg" className="w-full justify-start" onClick={() => setShowMsg(true)}>
          <MessageSquareText aria-hidden /> Add a message
        </Button>
      )}

      <Button variant={shot ? "soft" : "outline"} size="lg" className="w-full justify-start" onClick={() => setShot((s) => !s)} aria-pressed={shot}>
        {shot ? <Check aria-hidden /> : <Camera aria-hidden />} {shot ? "Screenshot attached (demo)" : "Attach screenshot"}
      </Button>
...
function StopView({ location, onContact, onBack }: { location: SupportLocation | null; onContact: () => void; onBack: () => void }) {
  return (
    <div className="space-y-4 pt-4">
      <LocationCard location={location} />
      <p className="text-base font-semibold">Leave your device exactly as it is. Don't restart it, and don't guess at the next step.</p>
      <div className="grid gap-2">
...
function SentView({ onClose }: { onClose: () => void }) {
  return (
    <div className="space-y-5 text-center">
      <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 300, damping: 18 }} className="mx-auto flex h-20 w-20 items-center justify-center rounded-full bg-success text-success-foreground">
